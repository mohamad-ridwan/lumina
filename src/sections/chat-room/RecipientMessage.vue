    ...
    <MessageReaction :message-deleted="!messageDeleted" wrapper-class="justify-end flex-row-reverse"
      ...
      <div ref="boxRef" class="group bg-[#f1f1f1] rounded-2xl max-w-[65%] md:max-w-[350px] self-start relative"
        ...
        <ImageMessage v-if="document?.type && !messageDeleted"
          :info="{ url: document.url, thumbnail: document?.thumbnail, caption: document?.caption, username: chatRoom.username, latestMessageTimestamp: Number(latestMessageTimestamp), hours: dayjs(Number(latestMessageTimestamp)).format('HH.mm'), messageId, profileId }"
