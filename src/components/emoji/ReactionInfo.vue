...
const { reactions, profileId } = defineProps(['reactions', 'profileId'])
...
<template>
  <div class="flex pb-1.5">
    <div class="bg-white rounded-full pr-2 py-[2px] flex items-center gap-2"
      :class="reactions.length > 1 ? 'pl-0' : 'pl-1'">
      <div class="flex gap-1">
        <button v-for="react of emojis" class="text-[15px] rotate-180">{{ react.emoji }}</button>
      </div>
